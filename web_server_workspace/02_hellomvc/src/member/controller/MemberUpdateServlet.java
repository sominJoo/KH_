...
//		request.setCharacterEncoding("UTF-8");//대소문자 상관없음. 요청한 view단의 charset값과 동일해야 한다.
		...
		Member member = new Member(memberId, null, memberName, MemberService.MEMBER_ROLE, gender, birthday, email, phone, address, hobby, null);
		...
		if(result>0){
			...
			session.setAttribute("loginMember", memberService.selectOne(memberId)); 		//변경된 member 정보를 session에 저장
		}
		else 
			msg = "회원정보수정에 실패했습니다.";	
		
		session.setAttribute("msg", msg);
		response.sendRedirect(request.getContextPath() + "/member/memberView"); // redirect시 mvc로 감 =>memberView로 지정
	}

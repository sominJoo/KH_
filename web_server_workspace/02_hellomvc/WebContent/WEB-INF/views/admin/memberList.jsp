	...
	<div id="search-container">
        ...
        <div id="search-memberId" class="search-type">
            <form action="<%=request.getContextPath()%>/admin/memberFinder">
                ...
                <input type="text" name="searchKeyword"  size="25" value="<%= "memberId".equals(type) ? keyword : "" %>" placeholder="검색할 아이디를 입력하세요."/>
        ...
        <div id="search-memberName" class="search-type">
            <form action="<%=request.getContextPath()%>/admin/memberFinder">
                ...
                <input type="text" name="searchKeyword" size="25" value="<%= "memberName".equals(type) ? keyword : "" %>" placeholder="검색할 이름을 입력하세요."/>
                <button type="submit">검색</button>			
            </form>	
        </div>
        <div id="search-gender" class="search-type">
            <form action="<%=request.getContextPath()%>/admin/memberFinder">
                ...
                <input type="radio" name="searchKeyword" value="M" <%= "gender".equals(type) && "M".equals(keyword) ? "checked" : "" %>> 남
                <input type="radio" name="searchKeyword" value="F" <%= "gender".equals(type) && "F".equals(keyword) ? "checked" : "" %>> 여
                <button type="submit">검색</button>
            </form>
        </div>
    </div>
...
$(".member-role").on("change", function(){
	...
	if(confirm(memberId +"님의 회원권한을 " + memberRoleStr+ "로 변경하시겠습니까?")){

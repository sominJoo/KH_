...
public class ResultFrame extends JFrame{	
	...
	public ResultFrame() {
		...
		for(int i =0 ; i< resultList.size() ;i++) {
			...
			menuQuantity.setFont(new Font("맑은 고딕",Font.BOLD, 25));
			...
			menuPrice.setFont(new Font("맑은 고딕",Font.BOLD, 25));
		...
		resultBtn.setFont(new Font("맑은 고딕",Font.BOLD, 15));
	...
	public void salesMenu() {		
		Cafeteria.salesList.add(new SalesMenu("비빔밥",5,30000));
		Cafeteria.salesList.add(new SalesMenu("라면",10,30000));
		Cafeteria.salesList.add(new SalesMenu("컵밥",10,40000));
		Cafeteria.salesList.add(new SalesMenu("제육볶음",2,16000));
		Cafeteria.salesList.add(new SalesMenu("김치찌개",3,15000));
		...
		int num = Cafeteria.menuList.size();
		String[] menuName = new String[num];
		//메뉴이름 확인용 배열
		for(int i=0 ; i< num; i++) {
			menuName[i] = Cafeteria.menuList.get(i).getMenu();
		}
		
		//결과값
		for(int i =0 ;i<Cafeteria.menuList.size(); i++) {
			int totalPrice = 0;
			int totalQuantity =0;
			//menuName[0]일때 salesList 검사
			for(int j =0 ; j<Cafeteria.salesList.size() ; j++) {
				String salesMenuName = Cafeteria.salesList.get(j).getMenu();
				if(menuName[i].equals(salesMenuName)) {
					totalPrice += Cafeteria.salesList.get(j).getPrice();
					totalQuantity += Cafeteria.salesList.get(j).getQuantity();
				}
			}
